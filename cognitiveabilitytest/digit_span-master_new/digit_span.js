...
async function experimentInit() {
  ...
  instructions = new visual.TextStim({
    win: psychoJS.window,
    name: 'instructions',
    text: '欢迎参加数字记忆大挑战！\n\n在这个任务中，你需要集中注意力，记住屏幕上快速闪现的一串数字。\n\n任务规则详解：任务从3位数字开始。在每个数字序列长度上，你都有3次尝试机会。\n只要你在3次尝试中能正确1次，数字序列就会增加1位，进入下一个难度。\n\n关于屏幕显示：数字会一个接一个地出现。在每个数字开始前，你会看到一个十字“+”号。\n请注意：这个“+”号是一个分隔符，用于提示你下一个数字即将出现。\n\n关于如何回答：当屏幕出现“回忆”字样时，请使用键盘上方的数字键，按顺序输入你记住的所有数字。\n输入时，请将数字紧挨着输入，中间不要加空格。\n\n举个例子：如果屏幕上依次显示的是“+”，“3“，”+“，”6“，”+“，”9”，\n那么你需要在回忆时，直接在键盘上输入“369”。如果输错了，可以按退格键删除后重新输入。\n\n每次尝试后，你都会立刻得到反馈，知道自己是否记对了。\n\n准备好了吗？请点击下面的按钮开始挑战吧！',
    font: 'Arial',
    units: undefined, 
...
function InstructionsRoutineEachFrame() {
  return async function () {
    ...
    if (cont_button_2.status === PsychoJS.Status.STARTED) {
    }
...
function PresentationRoutineEachFrame() {
  return async function () {
    ...
    if (background_2.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      ...
      // update status
      background_2.status = PsychoJS.Status.FINISHED;
    ...
    if (pres_text.status === PsychoJS.Status.STARTED) {
    }
    ...
    if (pres_text.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      ...
      // update status
      pres_text.status = PsychoJS.Status.FINISHED;
...
function PresentationRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        routineTimer.reset();} else if (PresentationMaxDurationReached) {
...
function RecallRoutineEachFrame() {
  return async function () {
    ...
    if (recall_txt.status === PsychoJS.Status.STARTED) {
    }
    ...
    if (cont_button.status === PsychoJS.Status.STARTED) {
    }
...
function EndRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    thank_you.setText(("\u4f60\u6240\u80fd\u8bb0\u4f4f\u7684\u6700\u957f\u6570\u5b57\u5e8f\u5217\u4e3a" + (last_level - 1).toString()));
...
function EndRoutineEachFrame() {
  return async function () {
    ...
    if (background_5.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      ...
      // update status
      background_5.status = PsychoJS.Status.FINISHED;
